...
// Simplified vendor copy: upstream's v0.159.0 probabilistic.go additionally
// consults the `processor.tailsamplingprocessor.usetracestate` feature gate
// (default: disabled) to optionally sample from the W3C tracestate `rv`/`th`
// fields instead of hashing the TraceID. Reading that gate requires
// `.../tailsamplingprocessor/internal/metadata`, an internal package this
// module cannot import either, so this copy keeps only the legacy
// hash-based path — i.e. exactly what v0.159.0 does with the gate at its
// default value. See ../NOTICE.md.
